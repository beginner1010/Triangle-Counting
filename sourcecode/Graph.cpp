...
int Graph::get_vertex_index(const long long& vertex) {
	if (this->vertex_index.find(vertex) == this->vertex_index.end()) {
		// vertex hasn't been seen already. Welcome to graph!
		this->vertex_index[vertex] = (int)this->vertices.size();
		this->vertices.push_back((int)this->vertices.size());
...
void Graph::preprocessing() {
	...
	std::unordered_set < long long > edge_mode;
	...
	while (std::fgets(char_line, 1024, IO::fin)) {
		...
		if (settings::interactive_mode == true) {
			if(settings::is_static == true)
				...
				print::message_with_dots(last_print_time, n_dots, cur_time, total_work, done_work, "loading graph stream");
		}
		line_number++;
		...
		if (((int)vec_str.size()) >= 2) {
			...
			if (is_all_num) {
				...
				long long vertex_left = helper_functions::to_ll(vec_str[0]);
				long long vertex_right = helper_functions::to_ll(vec_str[1]);

				if (vertex_left == vertex_right) // remove self loops
					continue;

				int A = this->get_vertex_index(vertex_left);
				int B = this->get_vertex_index(vertex_right);

				if (A > B) std::swap(A, B);
				long long edge_hash = this->encode_edge(A, B);

				if (seen_edges.find(edge_hash) == seen_edges.end()) {
					seen_edges.insert(edge_hash);
					edge_stream_ins_only.push_back({ A, B });
					this->add_new_edge(A, B, true);
					this->maximum_degree = mmax(this->maximum_degree,
						mmax((int)this->adj_vec[A].size(), (int)this->adj_vec[B].size()));
				}
				if (edge_mode.find(edge_hash) == edge_mode.end()) {
					edge_stream_fully_dynamic.push_back({ { A, B } , true });
					edge_mode.insert(edge_hash);
				}
				else {
					edge_stream_fully_dynamic.push_back({ { A, B } , false });
					edge_mode.erase(edge_hash);
				}
			}
		}
...
void Graph::reindex(std::vector < std::pair< long long, int > > &weights) {
	...
	// build ordered vectors
	std::vector <int> vertex_rank = std::vector <int>(this->n);
	for (int i = 0; i < this->n; i++)
		vertex_rank[weights[i].second] = i;
